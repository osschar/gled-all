...
public:
  ...
  // Common settings
  udword GetNbTriangles()	const	{ return mNbTris;  }
  udword GetNbVertices()	const	{ return mNbVerts; }
  void   SetNbTriangles(udword nb)	{ mNbTris = nb;    }
  void   SetNbVertices(udword nb)	{ mNbVerts = nb;   }
  ...
  RequestCallback GetCallback() const { return mObjCallback; }
#else
  // Pointers settings
  ...
  const	IndexedTriangle* GetTris()  const { return mTris;  }
  const	Point*		 GetVerts() const { return mVerts; }
  ...
  udword GetTriStride()    const { return mTriStride;    }
  udword GetVertexStride() const { return mVertexStride; }
#endif
#endif
   ...
   */
  //----------------------------------------------------------------------
  void	GetTriangle(VertexPointers& vp, udword index)	const
  {
#ifdef OPC_USE_CALLBACKS
