...
public:
   ...
   */
  //----------------------------------------------------------------------
  const	AABBTree* GetSourceTree() const { return mSource; }
   ...
   */
  //----------------------------------------------------------------------
  const	AABBOptimizedTree* GetTree() const { return mTree; }
   ...
   */
  //----------------------------------------------------------------------
  bool HasLeafNodes() const { return !(mModelCode & OPC_NO_LEAF); }
   ...
   */
  //----------------------------------------------------------------------
  bool IsQuantized() const { return mModelCode & OPC_QUANTIZED; }

  //----------------------------------------------------------------------
  /**
   *	Checks whether the model has a single node or not. This
   *	special case must be handled separately.
   *	\return		true if the model has only 1 node
   */
  //----------------------------------------------------------------------
  bool HasSingleNode() const { return mModelCode & OPC_SINGLE_NODE; }
   ...
   */
  //----------------------------------------------------------------------
  udword GetModelCode()	const { return mModelCode; }
   ...
   */
  //----------------------------------------------------------------------
  const	MeshInterface*	GetMeshInterface() const { return mIMesh; }
