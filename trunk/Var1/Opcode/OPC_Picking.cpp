...
bool Opcode::Picking(CollisionFace& picked_face, const Ray& world_ray,
                     ...
                     CullModeCallback callback, void* user_data)
{
  struct Local
  {
    ...
    static void RenderCullingCallback(const CollisionFace& hit, void* user_data)
    {
      ...
      // Discard face if hit point is smaller than min limit. This
      // mainly happens when the face is in front of the near clip
      // plane (or straddles it). If we keep the face nonetheless, the
      // user can select an object that he may not even be able to
      // see, which is very annoying.
  ...
  picked_face.mFaceID	= OPC_INVALID_ID;
  ...
  if(RC.Collide(world_ray, model, world))
    {
      return picked_face.mFaceID!=OPC_INVALID_ID;
    }
  return false;
}
