 ...
 *	- original code by Ville Miettinen, from Umbra/dPVS
 *        (released on the GD-Algorithms mailing list)
 *	- almost used "as-is", I even left the comments
 *        (hence the frustum-related notes)
 *
...
//----------------------------------------------------------------------
inline_ bool
PlanesCollider::PlanesAABBOverlap(const Point& center, const Point& extents,
                                  udword& out_clip_mask, udword in_clip_mask)
{
  // Stats
  mNbVolumeBVTests++;
      ...
      if(in_clip_mask & Mask)	// if clip plane is active, process it.
        {
          ...
          float NP = extents.x*fabsf(p->n.x) + extents.y*fabsf(p->n.y) + extents.z*fabsf(p->n.z);
