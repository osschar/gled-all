 ...
 *	\param		worldb		[in] OBB's world matrix, or 0
 *	\param		worldm		[in] model's world matrix, or 0
  ...
  if(TemporalCoherenceEnabled())
    {
      ...
      if(FirstContactEnabled())
        {
          // We're only interested in the first contact found => test
          // the unique previously touched face
          if(mTouchedPrimitives->GetNbEntries())
            {
              // Get index of previously touched face = the first
              // entry in the array
              udword PreviouslyTouchedFace = mTouchedPrimitives->GetEntry(0);

              // Then reset the array:
              // - if the overlap test below is successful, the index
              //   we'll get added back anyway
              // - if it isn't, then the array should be reset anyway
              //   for the normal query
              mTouchedPrimitives->Reset();

              // Perform overlap test between the cached triangle and
              // the box (and set contact status if needed)
      ...
      else
        {
          ...
          // We're interested in all contacts =>test the new real box
          // N(ew) against the previous fat box P(revious):
          if(IsCacheValid(cache) && TestBox.IsInside(cache.FatBox))
          ...
          else
            {
              ...
              // Make a fat box so that coherence will work for
              // subsequent frames
  ...
  for(udword i=0;i<3;i++)
    {
      for(udword j=0;j<3;j++)
        {
          // Epsilon value prevents floating-point inaccuracies
          // (strategy borrowed from RAPID)
...
//----------------------------------------------------------------------
/**
 *	Checks the OBB completely contains the box. In which case we
 *	can end the query sooner.
