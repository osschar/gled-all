...
protected:
  ...
  Float_t	mNearClip;	// X{GS} 7 Value(-width=>5, -range=>[0,1000,1,100],-join=>1)
  Float_t	mFarClip;	// X{GS} 7 Value(-width=>5, -range=>[0,1000,1,100])
  ...
  Float_t	mMSRotFac;	// X{GS} 7 Value(-range=>[-1000,1000,1,10],-width=>5,-join=>1)
  Float_t	mMSMoveFac;	// X{GS} 7 Value(-range=>[-1000,1000,1,10],-width=>5)

  Float_t	mCHSize;	// X{GS} 7 Value(-range=>[0,1,1,100], -width=>4,-join=>1)
  Bool_t	bShowRPS;	// X{GS} 7 Bool()
