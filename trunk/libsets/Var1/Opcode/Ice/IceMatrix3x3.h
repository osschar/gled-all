...
public:
  ...
  //! Copy constructor
  Matrix3x3(const Matrix3x3& mat) { CopyMemory(m, &mat.m, 9*sizeof(float)); }
  //! Destructor
  ~Matrix3x3() {}
  ...
  void		Copy(const Matrix3x3& source)	{ CopyMemory(m, source.m, 9*sizeof(float)); }

  // Row-column access
  //! Returns a row.
  void	        GetRow(const udword r, Point& p) const	{ p.x = m[r][0];  p.y = m[r][1];  p.z = m[r][2]; }
  //! Returns a row.
  const Point&	GetRow(const udword r) const { return *(const Point*)&m[r][0]; }
  //! Returns a row.
  Point&	GetRow(const udword r)       { return *(Point*)&m[r][0]; }

  //! Sets a row.
  void		SetRow(const udword r, const Point& p) { m[r][0] = p.x;  m[r][1] = p.y;  m[r][2] = p.z; }
  //! Returns a column.
  void		GetCol(const udword c, Point& p) const { p.x = m[0][c];  p.y = m[1][c];  p.z = m[2][c]; }
  //! Sets a column.
  void		SetCol(const udword c, const Point& p) { m[0][c] = p.x;  m[1][c] = p.y;  m[2][c] = p.z; }
  ...
  void	SkewSymmetric(const Point& a)
  {
    m[0][0] = 0.0f;
    m[0][1] = -a.z;
    m[0][2] = a.y;
  ...
  */
  ...
  Point		operator*(const Point& v) const { return Point(GetRow(0)|v, GetRow(1)|v, GetRow(2)|v); }
  ...
  {
    return Matrix3x3(s*mat.m[0][0],  s*mat.m[0][1],  s*mat.m[0][2],
