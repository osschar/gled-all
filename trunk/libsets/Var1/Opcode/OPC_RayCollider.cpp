 ...
 *
 *	In Opcode, we support segment queries, which yield half-line queries by
 *      setting d = +infinity. We don't support line-queries. If you need them,
 *      shift the origin along the ray by an appropriate margin.
 *
 *	In short, the lower bound is always 0, and you can setup the higher
 *      bound "d" with RayCollider::SetMaxDist().
 *
 ...
 *	- In "first contact" mode we return as soon as the ray hits one face.
 *        If can be useful e.g. for shadow feelers, where you want to know
 *        whether the path to the light is free or not (a boolean answer is enough).
 ...
 *        MT, 2.5.2009: Why? This really should not matter, as the triangle index
 *        is passed in by the user ... and is thus selected by the user.
 *        This condition has been commented out in InitQuery().
 *
 *	- If temporal coherence is enabled, the previously hit triangle is
 *        cached during the first query. Then, next queries start by colliding
 *        the ray against the cached triangle. If they still collide, we
 *        return immediately.
 *
 *	CLOSEST HIT:
 *      ===========
...
const char* RayCollider::ValidateSettings()
{
  ...
  // MT: commented out. This is explicitly done in the AABBTree* Collide() variant,
  // where this requirement is really claimed to be relevant.
...
//----------------------------------------------------------------------
/**
 *	Generic stabbing query for generic OPCODE models. After the call,
 *      access the results:
 ...
 *	\warning	SCALE NOT SUPPORTED. The matrices must contain
 *                      rotation & translation parts only.
 */
//----------------------------------------------------------------------
bool RayCollider::Collide(const Ray& world_ray,   const Model& model,
                          const Matrix4x4* world, udword* cache)
{
  // Checkings
 ...
 *
 *	\param		world_ray	[in] stabbing ray in world space
 *	\param		world		[in] object's world matrix, or 0
...
bool RayCollider::InitQuery(const Ray& world_ray, const Matrix4x4* world, udword* face_id)
{
  ...
  // MT: commented out first-contact requirement.
  if(TemporalCoherenceEnabled() && /*FirstContactEnabled() &&*/ face_id && *face_id!=OPC_INVALID_ID)
  {
