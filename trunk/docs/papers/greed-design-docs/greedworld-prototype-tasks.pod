...
This should help to relating the tasks to existing code.
...
three shared libraries: base (Glasses/, Stones/), GUI (Views/) and GL
...
Triangulated Surfaces library) and C<Audio1> (interface to
OpenAL, not yet used). All libsets require C<GledCore>.
...
Dynos (objects of glass C<Dynamico>) slide over terrain with given
...
If the units are coming closer, they are elastically scattered with conservation of
energy and momentum in horizontal plane. If they are going apart, they
are just displaced to avoid further contact.
...
Not much status, just some pointers.
...
Nothing. The flyers in the demo trivially keep at the same height
above the terrain.
...
=item Implement dynamics for movement of spece units with engine trust
and gravity.
...
=item<Eberly-3DGED> Eberly D.H., 3D Game Engine Design, 2nd edition.

=item<Tadel-GledIHSCM> Tadel M., \gled -- an Implementation of a
Hierarchic Server--Client Model, L<http://www.gled.org/docs/>.

=item<Terdiman-OPCODE> Terdiman P., OPCODE -- Optimized Collision
Detection library.
