...
{
  if(!callback)	return SetIceError("MeshInterface::SetCallback: callback pointer is 0");
...
{
  if(!tris || !verts)
    return SetIceError("MeshInterface::SetPointers: pointer is 0", 0);
...
{
  if(tri_stride<sizeof(IndexedTriangle))	return SetIceError("MeshInterface::SetStrides: invalid triangle stride", 0);
  if(vertex_stride<sizeof(Point))		return SetIceError("MeshInterface::SetStrides: invalid vertex stride", 0);
