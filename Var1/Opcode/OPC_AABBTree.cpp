...
/**
 ...
 optimized tree. If the primitive is a box, the resulting tree can be
 used for culling - VFC or occlusion -, assuming you cull on a
 mesh-by-mesh basis (modern way).
...
udword AABBTreeNode::Split(udword axis, AABBTreeBuilder* builder)
{
  ...
  udword NbPos = 0;
  // Loop through all node-related primitives. Their indices range
  // from mNodePrimitives[0] to mNodePrimitives[mNbPrimitives-1].
  ...
  for(udword i=0;i<mNbPrimitives;i++)
    {
      ...
      // Test against the splitting value. The primitive value is
      // tested against the enclosing-box center.
 ...
 *
 *	Note a perfectly-balanced tree is not well-suited to collision
 *	detection anyway.
 *
 *	\param		builder		[in] the tree builder
 ...
 */
//----------------------------------------------------------------------
AABBTree::AABBTree() : mIndices(0), mTotalNbNodes(0), mPool(0)
{}
...
bool AABBTree::Build(AABBTreeBuilder* builder)
{
  ...
  // For complete trees, check the correct number of nodes has been
  // created [Opcode 1.3]
 ...
 *	A well-balanced tree should have a log(n) depth. A degenerate
 *	tree O(n) depth.
...
{
  return Walk(0, 0);	// Use the walking code without callback
}
 ...
 *	A complete tree is made of 2*N-1 nodes, where N is the number
