...
public:
  ...
  //! Copy constructor
  Matrix4x4(const Matrix4x4& mat) { CopyMemory(m, &mat.m, 16*sizeof(float)); }
  //! Destructor.
  ~Matrix4x4() {}

  //! Assign values (rotation only)
  Matrix4x4& Set(float m00, float m01, float m02,
  ...
  //! Assign values
  Matrix4x4& Set(float m00, float m01, float m02, float m03,
                 float m10, float m11, float m12, float m13,
                 float m20, float m21, float m22, float m23,
                 float m30, float m31, float m32, float m33)
  {
  ...
  void	Copy(const Matrix4x4& source) { CopyMemory(m, source.m, 16*sizeof(float)); }

  // Row-column access
  //! Returns a row.
  void	GetRow(const udword r, HPoint& p) const { p.x=m[r][0]; p.y=m[r][1]; p.z=m[r][2];  p.w=m[r][3]; }
  //! Returns a row.
  void	GetRow(const udword r, Point& p)  const { p.x=m[r][0]; p.y=m[r][1]; p.z=m[r][2]; }
  //! Returns a row.
  const HPoint&	GetRow(const udword r)    const { return *(const HPoint*)&m[r][0]; }
  //! Returns a row.
  HPoint&	GetRow(const udword r)		{ return *(HPoint*)&m[r][0]; }

  //! Sets a row.
  void	SetRow(const udword r, const HPoint& p)	{ m[r][0]=p.x; m[r][1]=p.y; m[r][2]=p.z; m[r][3]=p.w; }
  //! Sets a row.
  void	SetRow(const udword r, const Point& p)	{ m[r][0]=p.x; m[r][1]=p.y; m[r][2]=p.z; m[r][3]= (r!=3) ? 0.0f : 1.0f; }
  //! Returns a column.
  void	GetCol(const udword c, HPoint& p) const	{ p.x=m[0][c]; p.y=m[1][c]; p.z=m[2][c]; p.w=m[3][c]; }
  //! Returns a column.
  void	GetCol(const udword c, Point& p)const	{ p.x=m[0][c]; p.y=m[1][c]; p.z=m[2][c]; }
  //! Sets a column.
  void	SetCol(const udword c, const HPoint& p)	{ m[0][c]=p.x; m[1][c]=p.y; m[2][c]=p.z; m[3][c]=p.w; }
  //! Sets a column.
  void	SetCol(const udword c, const Point& p)	{ m[0][c]=p.x; m[1][c]=p.y; m[2][c]=p.z; m[3][c]= (c!=3) ? 0.0f : 1.0f;	}

  // Translation
  //! Returns the translation part of the matrix.
  const HPoint&	GetTrans() const { return GetRow(3); }
  //! Gets the translation part of the matrix
  void	GetTrans(Point& p) const  { p.x=m[3][0]; p.y=m[3][1]; p.z=m[3][2]; }
  //! Sets the translation part of the matrix, from a Point.
  void	SetTrans(const Point& p)  { m[3][0]=p.x; m[3][1]=p.y; m[3][2]=p.z; }
  //! Sets the translation part of the matrix, from a HPoint.
  void	SetTrans(const HPoint& p) { m[3][0]=p.x; m[3][1]=p.y; m[3][2]=p.z; m[3][3]=p.w;	}
  //! Sets the translation part of the matrix, from floats.
  void	SetTrans(float tx, float ty, float tz) { m[3][0]=tx; m[3][1]=ty; m[3][2]=tz; }

  // Scale
  //! Sets the scale from a Point. The point is put on the diagonal.
  void	SetScale(const Point& p)        	{ m[0][0]=p.x; m[1][1]=p.y; m[2][2]=p.z; }
  ...
  /*
  //! Returns a row.
  HPoint GetRow(const udword row) const { return mRow[row]; }
  //! Sets a row.
  Matrix4x4& SetRow(const udword row, const HPoint& p) { mRow[row] = p; return *this; }
  //! Sets a row.
  ...
  */
  ...
  float	Trace3x3() const { return m[0][0] + m[1][1] + m[2][2]; }
  //! Clears the matrix.
  void	Zero()     { ZeroMemory(&m,  sizeof(m)); }
  //! Sets the identity matrix.
  void	Identity() { Zero(); m[0][0] = m[1][1] = m[2][2] = m[3][3] = 1.0f; }
  //! Checks for identity
  ...
  HPoint operator*(const HPoint& v) const { return HPoint(GetRow(0)|v, GetRow(1)|v, GetRow(2)|v, GetRow(3)|v); }
  ...
  friend Matrix4x4 operator*(float s, const Matrix4x4& mat)
  {
    return Matrix4x4(s*mat.m[0][0], s*mat.m[0][1], s*mat.m[0][2], s*mat.m[0][3],
