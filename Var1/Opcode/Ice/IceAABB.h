...
public:
  ...
  void	GetMin(Point& min) const { min = mMin; }
  //! Get max point of the box
  void	GetMax(Point& max) const { max = mMax; }

  //! Get component of the box's min point along a given axis
  float	GetMin(udword axis) const { return mMin[axis]; }
  //! Get component of the box's max point along a given axis
  float	GetMax(udword axis) const { return mMax[axis]; }

  //! Get box center
  void	GetCenter(Point& center)   const { center = (mMax + mMin)*0.5f;  }
  //! Get box extents
  void	GetExtents(Point& extents) const { extents = (mMax - mMin)*0.5f; }

  //! Get component of the box's center along a given axis
  float	GetCenter(udword axis)  const { return (mMax[axis] + mMin[axis])*0.5f; }
  //! Get component of the box's extents along a given axis
  float	GetExtents(udword axis) const { return (mMax[axis] - mMin[axis])*0.5f; }

  //! Get box diagonal
  void	GetDiagonal(Point& diagonal) const { diagonal = mMax - mMin; }
   ...
   */
  //----------------------------------------------------------------------
  bool	Intersect(const AABB& a, udword axis) const
  {
  ...
  AABB&	operator+=(const Point& trans)
  {
    mMin+=trans;
    mMax+=trans;
    return *this;
...
public:
  ...
  { mCenter = (max + min)*0.5f; mExtents = (max - min)*0.5f; }
  ...
  //! Get min point of the box
  void	GetMin(Point& min)	const	{ min = mCenter - mExtents; }
  //! Get max point of the box
  void	GetMax(Point& max)	const	{ max = mCenter + mExtents; }

  //! Get component of the box's min point along a given axis
  float	GetMin(udword axis)	const	{ return mCenter[axis] - mExtents[axis];}
  //! Get component of the box's max point along a given axis
  float	GetMax(udword axis)	const	{ return mCenter[axis] + mExtents[axis];}

  //! Get box center
  void	GetCenter(Point& center) const   { center = mCenter;   }
  //! Get box extents
  ...
  {
    float t, e;
    t = mCenter.x - a.mCenter.x; e = a.mExtents.x + mExtents.x; if(AIR(t) > IR(e)) return false;
    t = mCenter.y - a.mCenter.y; e = a.mExtents.y + mExtents.y; if(AIR(t) > IR(e)) return false;
    t = mCenter.z - a.mCenter.z; e = a.mExtents.z + mExtents.z; if(AIR(t) > IR(e)) return false;
    return true;
  }
   ...
   */
  //----------------------------------------------------------------------
  bool	GomezIntersect(const AABB& a)
  {
  ...
  //! Operator for AABB *= float. Scales the extents, keeps same center.
  AABB&	operator*=(float s)	{ mExtents*=s;	return *this;	}
