...
//! Count the number of '1' bits in a 32 bit word (from Steve Baker's
...
// computed by a SWAR algorithm that recursively "folds" the upper
// bits into the lower bits. This process yields a bit vector with the
// same most significant 1 as x, but all 1's below it. Adding 1 to
// that value yields the next largest power of 2. For a 32-bit value:
...
// u/(nbu*nbv) is 0 since u/nbu was 0 already.
// v/nbv is 0 as well for the same reason.
// Hence w = i/(nbu*nbv)
