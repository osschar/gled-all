...
//----------------------------------------------------------------------
/**
 *	Contains an implementation of the sweep-and-prune algorithm
 *	(moved from Z-Collide)
...
public:
  SAP_Element()	{}
  SAP_Element(udword id, SAP_Element* next) : mID(id), mNext(next) {}
  ~SAP_Element() {}
...
public:
  ...
  SAP_EndPoint*	Previous;	// Previous EndPoint whose Value is smaller than ours (or 0)
  SAP_EndPoint*	Next;		// Next EndPoint whose Value is greater than ours (or 0)
  ...
  if(mFirstFree)
    {
      // Recycle
      FreeElem = mFirstFree;
      mFirstFree = mFirstFree->mNext;	// First free = next free (or 0)
    }
  else
    {
  ...
  if(!Current)
    {
      ...
      mArray[id1] = GetFreeElem(id2, 0);
    }
  else if(Current->mID>id2)
    {
      // The list is not empty but all elements are greater than id2
      // => insert id2 in the front.
      mArray[id1] = GetFreeElem(id2, mArray[id1]);
    }
  else
    {
      // Else find the correct location in the sorted list (ascending
      // order) and insert id2 there.
      while(Current->mNext)
        {
  ...
  for(udword Axis=0;Axis<3;Axis++)
    {
      ...
      SAP_EndPoint* PreviousEndPoint = 0;
