  ...
  Fl_Button(x,y,w,h,t), fImg(img), fData(dt)
{
  ...
  label(fData.fLabel.c_str());
  if(fData.fDndP) labelcolor(fl_rgb_color(0,10,50));
  tooltip(fData.fTooltip.c_str());
}
...
{
  static const string _eh("MCW_Button::handle ");
  ...
  case FL_PASTE: {
    ...
    if(beta_id != 0) {
      auto_ptr<MCW_View> v( shell->MakeMCW_View(fImg, fData.fMInfo) );
      if(v.get() != 0) {
	v->SetABG(fImg->fGlass->GetSaturnID(), beta_id);
	v->Send();
      }
    }
    return 1;
