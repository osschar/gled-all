...
struct OPCODE_API BuildSettings
{
  ...
  udword mLimit; //!< Limit number of primitives / node. 
                 //!< If limit is 1, build a complete tree (2*N-1 nodes)
...
public:
  ...
  //----------------------------------------------------------------------
  /**
   *	Validates node subdivision. This is called each time a node is
   *	considered for subdivision, during tree building.
   *	\param	primitives	[in] list of indices of primitives
   *	\param	nb_prims	[in] number of indices
   *	\param	global_box	[in] global AABB enclosing the set of input primitives
  ...
  // Stats
  void   SetCount(udword nb)      { mCount=nb;     }
  void   IncreaseCount(udword nb) { mCount+=nb;    }
  udword GetCount() const         { return mCount; }
  void   SetNbInvalidSplits(udword nb)  { mNbInvalidSplits=nb;     }
  void   IncreaseNbInvalidSplits()	{ mNbInvalidSplits++;      }
  udword GetNbInvalidSplits()	const	{ return mNbInvalidSplits; }
...
public:
  //! Constructor
  AABBTreeOfVerticesBuilder() : mVertexArray(0)	{}
  //! Destructor
...
public:
  //! Constructor
  AABBTreeOfAABBsBuilder() : mAABBArray(0) {}
  //! Destructor
...
public:
  //! Constructor
  AABBTreeOfTrianglesBuilder() : mIMesh(0) {}
  //! Destructor
