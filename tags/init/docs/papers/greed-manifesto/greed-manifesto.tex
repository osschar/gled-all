...
% Family: Alja, Vid, (? Bostjan, Fata, Tomo)
%
% Friends: Jona, Klemen, Ziga, Iggy, Siso, Joh, (? Toncek)
%
% IJS: Mare, Kerso, Filipko, Miso, Cric, Johhny,
%
% ALICE / ROOT: Federico, Bertrand, Axel, Fons, Rene, Predrag,
% (? Andrei, Cvetan, Peter, Latchezar, Jan-Fiete, Kostin, Svetozar)
% Derek, Andreas,
% Timur, Artem,
% Jochen, Volker,
%
...
a project, unless it is an acronym (in order of relevance):
\begin{itemize}\tightlist

\item Global Research Environment for Equitable Development

\item Global Research Environment for Eidetic Design

\item Generic Religious Engine for Ego Deification
...
Every global initative starts with a concrete problem and with a
...
last five years\footnote{Currently folding@home is the most acive one,
...
they are contributibng to. The idea of \greed encompasses all of the
...
and of massive multiplayer online games (World of Warcraft, EVE
...
structure must be employed. It must be general enough to allow a
...
To underline the fragility of our civilization, the story begins with
a cataclysm: Human population of Earth is decimated by a global
disaster (caused by biological agents, environmental collapse or LHC
blow-up) and a small number of survivors is in position to operate
robotic factories and agents. They are used to build Foundation
vessels that are sent through poorly controlled wormholes into random
solar systems of our universe where they have to bootstrap new
civilizations. Of course, the users themselves will determine for each
individual world if its further development will be carried out in a
cooperative or in an opposing manner.
...
can also be gained via online quizzes where a user is given a
specific data-set on which he must perform data-analysis and
correctly reply to a set of questions. For major scientific and
technological breakthroughs the research cost far exceeds the
computing resources of a single user, thus forcing the users to
form cooperative communities.
...
Astronomy and cosmology, the sister sciences of the high-energy
physics, strive for the understanding of the universe and of all the
phenomena that proceed within it on a wide range of spacetime-scales.
It is therefore necessary to include the astronomical facts, at least
at a basic level, into the world mechanics from the very beginning.
With the interest from the astro-physics community, detailed knowledge
about universe could be incorporated into the world mechanics, from
realistic taxonomy and composition of solar and planetary systems to
large-scale structure of galaxies, galactic clusters and beyond.
...
hypothetical couplings between GR and QM could be stipulated and
explored in semi-realistic technological contexts leading to, for
example, faster then light travel and space-time manipulation devices.
...
\begin{itemize}\tightlist
\item \emph{Scope and detail of scientific, tecnological and
...
\item \emph{Spatial extent, duration and tempo} pose the requirements
  ...
  accomplish sizeable advancement.
\end{itemize}
...
\item testing of new world mechanisms and their balancing against
  existing elements.
\end{enumerate}
As these worlds are relativel short-lived, they can be regularly
tailored to serve the changing needs of the community.

\paragraph{Intermediate worlds} combine more elements on a higher
level of detail and ther duration ranges from months to years. These
...
This section analyzes benefits and concessions of differnet parties
...
\item Understanding of contemporary research activites and technology.
...
\item Involvment in world creation and world maintenace.
...
Until now, commerical corporations have not been mentioned as possible
...
spaceships, Hyundai factory control systems, or Novartis life
extension drugs. To prevent complete commercialization of \greed, the
participation of commercial entities should be restricted to a subset
of worlds that offer additional benefits to the users in exchange for
being targets of advertising campaigns.
...
\item Advertising, in both senses mentioned above. First, specific
  ...
  second, boosting brand and trademark recogition through in-world products
  or entities.
...
\item Providing in-world or real-world rewards for users that meet
  ...
  improvement of in world entites or mechanisms.
...
sub-elements of \greed are individully reviewed in both software and
hardware terms.
...
can be used for proceessing of all data related to \greed functioning,
e.g., analysis of infrastructure performance data or preparation of
in-world statistcs. As users will be required to analyse world data to
fully understand and explore the possibilites of individual worlds,
...
(possibly based on LDAP with Postgres back-end and OpenCA) will have
to be implemented, especially since it will be accessed by almost all
elements of \greed computing: community web portals, \greedhome and
\greed-world servers and clients as well as \alien and \greed
accounting systems.
...
\greed-worlds are operated by a set loosely connected servers, each of
them steering a particular region of a world, ranging in size from a
solar system to a well defined region of a given planet or moon. While
a single server per planet can be used initially, it is of great
...
maintainer interface should be available as plugins.
...
allows for an easy inclusion of intermediatre proxy nodes that reduce
...
With the LHC startup right behind the corner, the next year seems to
...
Probably LDAP with Postgres backend. \alien and \grid identity
...
\item Standard security level for developers, world maintiners and
...
computing resources and issuing of reserch creadits.  Probably easiest
...
\alien BITS build infrastructure seems reasonsble. Build for various
...
The goal is to demonstrate feasability of real-time evolution of
...
resurces and participate in \greed worlds.
...
completely imaginary ones to those that attempt to mimic our reallity.
...
buildings, factories, vehicles, AI agents and human specialists. Users can interact
with them and influence their future state or actions.
