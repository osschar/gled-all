 ...
 *	Usage:
 *
 *	1) Create a static mesh interface using callbacks or
 *	pointers. (see OPC_MeshInterface.cpp).  Keep it around in your
 *	app, since a pointer to this interface is saved internally and
...
bool Model::Build(const OPCODECREATE& create)
{
  ...
  if(create.mSettings.mLimit!=1)
    return SetIceError("OPCODE WARNING: supports complete trees only! Use mLimit = 1.\n", 0);
  ...
  Release(); // Discard previous tree [Opcode 1.3, thanks Adam]
  ...
  if(NbTris == 1)
    {
      // We don't need to actually create a tree here, since we'll
      // only have a single triangle to deal with anyway.
