...
MCW_View::ArgBase::ArgBase(string& typ, string& base_typ,
  ...
  _typ(typ), _base_typ(base_typ), _name(name)
{
  ...
  insert_box(FGS::swm_label_width(_typ, cell_w),  _typ.c_str());
  insert_box(FGS::swm_label_width(_name, cell_w), _name.c_str());
}
...
MCW_View::CtxArg::CtxArg(string& typ, string& base_typ,
			 string& name, string& defval,
			 int cell_w) :
...
MCW_View::VarArg::VarArg(string& typ, string& base_typ,
			 string& name, string& defval,
			 int cell_w) :
  ArgBase(typ, base_typ, name, defval, cell_w)
{
  static const string _eh("MCW_View::VarArg::VarArg ");

  _typ_idx = typ_idx(base_typ.c_str());
  // printf("For %s got type %d\n", base_typ.c_str(), _typ_idx);
  if(_typ_idx < 0)
    throw(_eh + GForm("type '%s' not supported.", base_typ.c_str()));
  ...
  case 1: {
    ...
    // printf("%s::%s -> %lf, %lf\n", base_typ.c_str(), name.c_str(),
    //   w->minimum(), w->maximum());
    w->step(at.stepA, at.stepB);
    if(!defval.empty()) {
      double x; sscanf(defval.c_str(), "%lf", &x);
      w->value(x);
    }
  ...
  case 3: {
    ...
    // w->callback((Fl_Callback*)s_Change_cb, FGS::grep_parent<MCW_View*>(this));
    if(!defval.empty()) {
      if(defval.compare("false") && defval.compare("0"))
	w->value(1);
    }
    break;
  }
...
{
  const static string _eh("MCW_View::VarArg::StreamData ");
  ...
  case 1: {
    ...
    default:{ printf("%sunexpected valuator type.", _eh.c_str()); break; }
  ...
  default: {
    printf("%sunexpected argument type.", _eh.c_str());
    break;
  }
  }
}
...
void MCW_View::ParseMethodInfo(GledNS::MethodInfo* mi) throw(string)
{
  mMInfo = mi;
  mTitle = GForm("%s::%s", mMInfo->fClassInfo->fName.c_str(), mMInfo->fName.c_str());
  label(mTitle.c_str());
