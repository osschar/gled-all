...
unsigned int TPME::DeterminePcreOptions ( const TString & opts ///< perl style character modifiers -- i.e. "gi" is global, case-insensitive
 )
{
	unsigned int return_opts = 0;
	if ( strchr ( opts.Data ( ), 'i' ) ) {
		return_opts |= PCRE_CASELESS;
	}
	if ( strchr ( opts.Data ( ), 'm' ) ) {
		return_opts |= PCRE_MULTILINE;
	}
	if ( strchr ( opts.Data ( ), 's' ) ) {
		return_opts |= PCRE_DOTALL;
	}
	if ( strchr ( opts.Data ( ), 'x' ) ) {
		return_opts |= PCRE_EXTENDED;
	}
	// not perl compatible
	if ( strchr ( opts.Data ( ), 'U' ) ) {
		return_opts |= PCRE_UNGREEDY;
	}

	// if 'g' is set, it stores the previous
	//   result end position
	if ( strchr ( opts.Data ( ), 'g' ) ) {
		m_isglobal = 1;
	}
...
void					
TPME::options(unsigned opts ///< PCRE-style option flags to be set on TPME object
)
{
	_opts = opts;
}
...
void 
TPME::compile(const TString & s ///< string to compile into regular expression
)
{
	const char * errorptr;
	int erroroffset;
	
	re = pcre_compile(s.Data(), _opts, &errorptr, &erroroffset, 0);
...
pcre *
TPME::clone_re(pcre * re ///< TPME object to clone
)
{
	if ( !re )
		return 0;
...
TPME::TPME(const TString & s ///< string to copmile into regular expression
	 , unsigned opts ///< PCRE-style option flags to be set on TPME object
)
{
	reset ( );
	m_isglobal = 0;
...
TPME::TPME ( const TString & s, ///< string to compile into regular expression
	   const TString & opts ///< perl-style character flags to be set on TPME object 
)
{
	reset ( );
	m_isglobal = 0;
...
TPME::TPME(const char * s, ///< string to compile into regular expression
	 unsigned opts ///< PCRE-style option flags to be set on TPME object
)
{
	reset ( );
...
TPME::TPME(const TPME & r ///< TPME object to make copy of 
)
{
	reset ( );
	m_isglobal = 0;
...
int
TPME::match(const TString & s, ///< s String to match against
...
//		fprintf ( stderr, "TPME RESETTING: new string\n" );
		lastglobalposition = 0;
	}
	...
	nMatches = pcre_exec(re, extra, s.Data(), s.Length(), offset, 0, m, msize);
...
TString
TPME::substr(const TString & s, const vector<TPME::markers> & marks,
			  unsigned index)
{
 ...
 */
int
TPME::split(const TString & s, unsigned maxfields)
{
...
TString TPME::operator[](int index)
{
...
TString TPME::UpdateReplacementString ( const TString & r ) {
	TString finalreplacement = r;
...
TString TPME::sub ( const TString & s, const TString & r,
		   int dodollarsubstitution )
{
	...
	if ( m_isglobal ) {
		...
		while ( match ( s ) ) {
			...
			newstream += s( endoflastmatch, m_marks[0].first - endoflastmatch );
		...
		// copy the last bit
		newstream += s( endoflastmatch, kNPOS );
		...
		if ( nMatches > 0 ) {
			...
			newstream += s ( 0, m_marks[0].first );
			newstream += finalreplacement;
			newstream += s ( m_marks[0].second );
...
StringVector TPME::GetStringVector ( ) 
{
