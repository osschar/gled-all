// $Header$

// Copyright (C) 1999-2003, Matevz Tadel. All rights reserved.
// This file is part of GLED, released under GNU General Public License version 2.
// For the licensing terms see $GLEDSYS/LICENSE or http://www.gnu.org/.

#ifndef Geom1_Rect_H
#define Geom1_Rect_H

#include <Glasses/ZNode.h>
#include <Stones/ZColor.h>

class Rect : public ZNode {
  // 7777 RnrCtrl("true, 7, RnrBits(2,4,6,0, 0,0,0,5)")
  MAC_RNR_FRIENDS(Rect);

private:
  void _init();

protected:
  Real_t	mULen;		// X{GS} 7 Value(-range=>[0.1,1000, 1,10], -join=>1)
  Real_t	mVLen;		// X{GS} 7 Value(-range=>[0.1,1000, 1,10])
  UShort_t	mUStrips;	// X{GS} 7 Value(-range=>[0,100,1], -join=>1)
  UShort_t	mVStrips;	// X{GS} 7 Value(-range=>[0,100,1])
  Float_t	mWidth;		// X{GS} 7 Value(-range=>[0,100,1,10])
  ZColor	mColor;		// X{PGS} 7 ColorButt()

public:
  Rect(Text_t* n="Rect", Text_t* t=0) : ZNode(n,t) { _init(); }

  void SetUV(Real_t u, Real_t v)
  { mULen = u; mVLen = v; Stamp(LibID(), ClassID()); }
  void SetUnitSquare(UShort_t size)
  { mULen = mVLen = size; mUStrips = mVStrips = size; Stamp(LibID(), ClassID()); }

#include "Rect.h7"
  ClassDef(Rect, 1)
}; // endclass Rect

GlassIODef(Rect);

#endif
