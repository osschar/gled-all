...
start adding objects into the system, the server must be spawned, as
in C<$GLEDSYS/macros/sun.C>:
...
with the distribution and reside in the C<$GLEDSYS/macros>
directory. Typically the demo macros execute C<sun.C> somwhere close
to the beginning and C<eye.C> (standard GUI) at the end.
...
External configuration is only needed for authentication support.
...
Some scripts might not start the GUI. They should be run as:
 ...
 gled -noauth <demo-name>.C

or, if the server/viewer is not started from the script:

 gled -noauth sun.C <demo-name>.C eye.C

The C<sun.C> macro starts the server (must precede the demo macro) and
C<eye.C> starts the default viewer (must follow the demo macro).

Note that you will need the C<-nauth> option (to disable
authentication) or C<-eyeid sun.absolute> (to spawn GUI with the
adminstrator privileges) if you want to edit the top-level
objects. Most demo scripts do not setup any authorization scheme and
thus GUI requests to top-level objects are bluntly denied.
...
=head2 Connecting to a running Gled

To connect to an already running gled, use:

 gled -r -m <host-to-connect-to> moon.C

C<moon.C> script will in principle just open the shell and a GL window
on the first scene.
...
failure. This should be fixed soon.
...
=item C<TRint> (C<ROOT>'s interactive application) processes macros given
on the command line. Note that the server is not allowing moon
connections yet and the scripts can freely create objects and attach
them into the object graphs without worrying about the clients.

=item Unless C<-nomoons> option is given, allow moon connections. This can
...
This field could (well, should) be generalized into a special family
to allow specification of any timestamp variable that needs to be
updated by a cannonical name, e.g.

 Timestamps{Trans,Tring}
