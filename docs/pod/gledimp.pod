...
Both will list their command-line options if started with C<-h> or C<--help>
switch:
...
servers at C<www.gled.org>) so network connection is
required. Obviously, you will need C<subversion> client and C<GNU
autotools>. C<CMake> is also needed for building of C<OpenAL>. Further,
...
The next section gives some examples (L<"Examples of data and function
meber declarations">), more can be found by browsing the code or
auto-generated class documentation
L<http://www.gled.org/docs/classdoc/>. The full list of all recognized
directives is in a dedicated section (L<"Specification of C<project7>
directives">).
...
=head1 Specification of C<project7> directives

C<project7> directives are parsed differently for data-members and for
methods. Further, they are grouped into several families, each
specified on the declaration line as C<familiy{options}> (see
examples in L<"Examples of data and function meber declarations">).


=head2 Directives for data-members
...
a pointer or a reference. Also, for I<link>-members, the
...
=item<sS> Auto generate C<void SetBar(Foo_t bar)>. With B<S> also
...
C<MIR* S_SetBar(Foo_t bar)>, like if the C<e|E> flag is specified.

If B<q|Q> is specified, the set method signature is C<SetFoo(Foo_t& bar)>
| C<SetFoo(const Foo_t& bar)>.
...
Auto-generate MIR producing C<MIR* S_SetBar(Foo_t bar)> but NOT the
C<void SetBar(Foo_t bar)> method -- it needs to be declared and
implemented by the user. C<E> and C<e> are equivalent.

If the first character on the comment line is 'C<!>' the method will
be tagged as I<local>: MIR will be sent to the local saturn only. This
is useful for updating or printing of local data members.

=item<dD>

Auto-generate the C<void DeltaBar(Foo_t bar)> method that calls
C<operator+> on current value and the argument. With B<D> also
generate the Stamp method, that will send update request to the
viewer.

B<Note:> the B<dD> flag for function-members requests member execution
in a detached thread.

=item<fF> 

Auto-generate MIR producing C<MIR* S_DeltaBar(Foo_t bar)> but NOT the
C<void DeltaBar(Foo_t bar)> method -- it needs to be declared and
implemented by the user. C<F> and C<f> are equivalent.

This is not applicable for link data-members.

Same comments as for B<eE> apply.
...
=head3 Additional directives for link data-members
...
=item<aA> Generate assert methods that check if link is set.

B<a> -- C<< protected: <link-type*> assert_<lower-case-link-name>(const Exc_t& exc_header); >>

B<A> -- C<< public: <link-type>* Assert<link-name>(const Exc_t& exc_header); >>

=item<fF> Assumes that data-member C<mLinkName_fid> exists. The link set
method does additional check if the argument matches to this C<FID>. By
convention, if B<f> is given the C<fid>-variable is not streamed and should be
set in C<_init()> method (or constructor). If C<F> is given, the
C<fid>-variable is streamed and it should be set in C<AdEnlightenment()>. This
...
See section L<"Rendering classes"> for explanation and syntax.

=back


=head2 Directives for function-members
...
auto-generate MIR producing C<MIR* S_FooBar(arguments)>. C<E> and C<e>
are equivalent.
...
To get the same behaviour for C<Set> methods one has to write them
manually and declare them like this:

 some_type   some_var;          // X{G}
 void Setsome_var(some_type x); // X{ED}

B<Note:> the B<dD> flag for data-members requests creation of I<Delta> method.
...
=head2 Directives valid for data and function-members
