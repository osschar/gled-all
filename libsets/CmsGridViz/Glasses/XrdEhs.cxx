  ...
  while (! b_stop_server)
  {
    ...
    for (list<XrdFile*>::iterator xfi = flist.begin(); xfi != flist.end(); ++xfi)
    {
      ...
      // if (mode == kAll)
      {
        ...
        oss << Form("<td>%s</td>", file->GetUser()->GetServer()->GetDomain()) << endl;
        oss << Form("<td>%s</td>", file->GetUser()->GetFromDomain()) << endl;
        oss << Form("<td>%s</td>", file->GetUser()->GetRealName()) << endl;

        oss << Form("<td>%.3f</td>", file->GetReadStats().GetSumX()) << endl;
