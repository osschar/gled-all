...
//----------------------------------------------------------------------
/**
 	Builds an implicit tree from a standard one. An implicit tree
 	is a complete tree (2*N-1 nodes) whose negative box pointers
 	and primitive pointers have been made implicit, hence packing
 	3 pointers in one.
...
{
  // Current node from input tree is "current_node". Must be flattened
  // into "linear[boxid]".
...
//----------------------------------------------------------------------
/**
 	Builds a "no-leaf" tree from a standard one. This is a tree
 	whose leaf nodes have been removed.
 ...
 */
//----------------------------------------------------------------------
AABBCollisionTree::AABBCollisionTree() : mNodes(0)
{
}
 ...
 */
//----------------------------------------------------------------------
AABBNoLeafTree::AABBNoLeafTree() : mNodes(0)
{
}
...
// - We could use the highest bits of mData to store some more
//   quantized bits. Dequantization code would be slightly more complex,
//   but number of overlap tests would be reduced (and anyhow those bits
//   are currently wasted). Of course it's not possible if we move to 16
//   bits mData.
// - Something like "16 bits floats" could be tested, to bypass the
//   int-to-float conversion.
// - A dedicated BV-BV test could be used, dequantizing while testing
//   for overlap. (i.e. it's some lazy-dequantization which may save
//   some work in case of early exits). At the very least some muls
//   could be saved by precomputing several more matrices. But maybe not
//   worth the pain.
// - Do we need to dequantize anyway? Not doing the extents-related
//   muls only implies the box has been scaled, for example.
// - The deeper we move into the hierarchy, the smaller the extents
//   should be. May not need a fixed number of quantization bits. Even
//   better, could probably be best delta-encoded.


// Find max values. Some people asked why I wasn't simply using the
// first node. Well, I can't.  I'm not looking for (min, max) values
// like in a standard AABB, I'm looking for the extremal
// centers/extents in order to quantize them. The first node would
// only give a single center and a single extents. While extents would
// be the biggest, the center wouldn't.
